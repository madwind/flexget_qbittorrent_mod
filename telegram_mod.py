...
ChatIdsBase = db_schema.versioned_base('telegram_mod_chat_ids', 0)
...
class ChatIdEntry(ChatIdsBase):
    __tablename__ = 'telegram_mod_chat_ids'
    ...
    * Execute: `flexget telegram test-msg`.
    ...
    def _send_msgs(self, msg, chat_ids):
        ...
        for chat_id in (x.id for x in chat_ids):
            try:
                msg_limits = self._get_msg_limits(msg)
                for msg_limit in msg_limits:
                    logger.debug('sending msg to telegram servers: {}', msg_limit)
                    self._bot.sendMessage(chat_id=chat_id, text=msg_limit, **kwargs)
